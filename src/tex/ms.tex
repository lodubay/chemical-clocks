...
\begin{abstract}
    ...
    We find that the slope of the correlation varies significantly with metallicity and guiding radius in the chemical thin disk. The trend is steepest in the outer disk and below Solar metallicity, while the most metal-rich stars and those in the inner disk show no correlation with age. Halo stars have higher [Ce/Mg] than thick disk, suggesting that AGB enrichment is important even at low metallicity. 
    Using a galactic chemical evolution model of the disk, we find that standard AGB yield predictions are unable to match the observed [Ce/Mg]--age trends, which require significantly longer delay times for Ce enrichment. By modifying the yields to shift Ce production to lower-mass AGB stars, we can qualitatively match thin disk abundances across the Galaxy. Due to the variation with environment and the lack of a strong theoretical foundation, we caution against the use of [Ce/Mg] as a chemical clock.
\end{abstract}
...
Literature notes: neutron capture abundance measurements
\begin{itemize}
    ...
    \item \citet{maiorca_s-processing_2011} found this trend also applies to Y, Zr, La, and Ce in open clusters. They note that such a trend requires a much higher contribution to $s$-process enrichment from very low mass AGB stars ($\lesssim1.5\Msun$) than expected from theoretical predictions.
    ...
    \item \citet{sales-silva_exploring_2022} looked at [Ce/Fe] and [Ce/$\alpha$] in open clusters (OCCAMS) and found a metallicity dependence in their age relations. They also found that the [Ce/$\alpha$]--age relation changes across the disk.
\end{itemize}

AGB theory
\begin{itemize}
    \item \citet{battino_mixing_2021} modeled the nucleosynthesis of low-mass AGB stars at low metallicity, and showed that mixing processes are critically important to $s$-process nucleosynthesis at low metallicity. They favored low- to no mixing and a large 13C pocket size when comparing to observations.
\end{itemize}
...
\begin{figure*}
    \centering
    \includegraphics[width=\textwidth]{figures/ce_lines.pdf}
    \caption{Ce II absorption lines for selected stars with $S/N\sim200$. The color and marker shape for each spectrum is consistent across all panels. From top to bottom, the 2-MASS IDs for the selected stars are: 2M09530814+6207498, 2M04364872+5016286, 2M01231384+6014386, 2M21314472-0350388, 2M15025976+7827316, and 2M09112399-4841166.}
    \label{fig:ce-lines}
\end{figure*}
