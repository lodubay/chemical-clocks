...
\begin{abstract}
    The $[s/\alpha]$ abundance ratio has been proposed as a ``chemical clock,'' or an empirical tracer of stellar age. However, the mechanism driving this relation is not well understood, and there may be substantial variations with metallicity and Galactic radius. Using Ce and Mg as representative elements, we quantify the $[s/\alpha]$--age correlation across the disk for over 100,000 stars in the Milky Way Mapper survey, a program of SDSS-V. We find that the slope of the correlation in the high-Ia population varies significantly with metallicity and guiding radius. The steepest trends are found in the outer disk and at slightly sub-Solar metallicity, while the most metal-rich stars and those in the inner disk show no correlation with age. Halo stars have higher [Ce/Mg] than low-Ia stars, suggesting that AGB enrichment is important even at low metallicity. Using a galactic chemical evolution model of the Milky Way disk, we find that standard AGB yield predictions are unable to match the observed [Ce/Mg]--age trends.
    The observed trend requires a delay time for Ce enrichment an order of magnitude longer than predicted by theoretical yields for AGB stars. By modifying the yields to shift Ce production to lower-mass AGB stars, we are able to qualitatively match abundance trends across the Galaxy for high-Ia stars. Due to the variation with environment and the lack of a strong theoretical foundation, we caution against the use of $[s/\alpha]$ as a chemical clock.
\end{abstract}
...
\begin{figure}
    ...
    \caption{Median age trends in bins of $R_{\rm guide}$ for high-Ia (solid) and low-Ia (dashed) populations. Semi-transparent bands represent the bootstrapped error on the median abundance in each age bin. Stars are restricted to $z_{\rm max}<0.5$ kpc.}
    \label{fig:median-age-trends}
\end{figure}

\todo{Discuss Figure \ref{fig:median-age-trends}.} Within the high-Ia population, both the [Ce/H] and $\Delta\bracket{Ce}$--age trends vary with $R_g$. The slope is steepest in the outer Galaxy ($R_g=13-15\kpc$) and flat in the inner Galaxy ($R_g=3-5\kpc$). \todo{Is the break at 6 Gyr ages important?}
