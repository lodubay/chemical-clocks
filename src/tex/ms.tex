...
\begin{figure*}
    ...
    \caption{Stellar abundance distributions in three different radial bins predicted by the model with $y/Z_\odot=2$, KL16+K18, and $M_{\rm AGB}\times0.5$. The layout is similar to Figure \ref{fig:median-trends-grid}. Each point represents a model stellar population, color-coded by age. For visual clarity, only a random sample of 10,000 points is plotted in each panel. The dashed curve plots the gas abundance track at the mean radius in each bin. The solid contours show the 10th, 30th, 50th, 70th, and 90th percentiles of the MWM data in each region.}
\end{figure*}
