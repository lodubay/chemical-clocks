...
\end{equation}
This results in a CCSN ``plateau'' of $\bracket[Mg]{Ce}_{\rm CC}=\log_{10}(f_{\rm Ce,\odot}^r) \approx -0.64$, akin to the low-Ia $\bracket[Fe]{\alpha}$ plateau.
For AGB stars, we adopt the yield grids of \citet{cristallo_evolution_2011,cristallo_evolution_2015}. \todo{Figure X shows the dependence of the Ce yield on the mass and metallicity of the AGB stars. The yield peaks sharply at $Z=X$ and the median enrichment time is Y.} We have also tested the yields of \citet{karakas_stellar_2016} and \citet{karakas_heavy-element_2018}. These produce roughly a factor of 2 more Ce, but after scaling our CCSN yields and outflows to match, they predict similar abundance evolution to our fiducial models.
