...
We adopt an exponential prescription for the outflow mass-loading factor:
\begin{equation}
    \eta \equiv \frac{\dot\Sigma_{\rm gas}}{\dot\Sigma_\star} = \eta_\odot \exp\Big(\frac{R_{\rm gal}-R_\odot}{R_\eta}\Big),
\end{equation}
where we take $\eta_\odot=0.4$ and $R_\eta=5\kpc$ to reproduce a radial metallicity gradient of $-0.06\dex\kpc^{-1}$ \citep{mendez-delgado_gradients_2022}. \citet{johnson_milky_2025} discuss the motivation behind this prescription and the evidence for mass-loaded outflows in detail.

For our fiducial model, we adopt the ``inside-out'' star formation history from \citet{johnson_stellar_2021}. This SFH can reproduce the \citet{feuillet_spatial_2019} stellar age--metallicity and age--[$\alpha$/Fe] relations across the disk, although it does not reproduce the Milky Way's $\alpha$-bimodality \citep{johnson_stellar_2021}. Our primary goal is to explain the Ce abundance patterns in the high-Ia population, but the lack of a distinct low-Ia disk is a limitation of our models. Star formation proceeds according to a \citet{schmidt_rate_1959} law $\dot\Sigma_\star\propto\Sigma_g^k$ with $k=1.5$ \citep{kennicutt_global_1998}, with the detailed implementation described by \citet{johnson_milky_2025}. The star formation rate in each zone is normalized to reproduce the double-exponential radial stellar density profile of \citet{bland-hawthorn_galaxy_2016}.
...
\begin{figure}
    ...
    \caption{Comparison of [Ce/Mg]--age trends predicted by multi-zone models. The blue curve represents the gas abundance evolution in fiducial model, and all other curves represent models that differ by one parameter. The gray filled (open) points represent low-Ia (high-Ia) stars in the Solar neighborhood ($7\leq R_g<9\kpc$, $z_{\rm max}<0.5\kpc$) with $-0.1\leq\bracket{Mg}<+0.1$. The gray error bars indicate the median uncertainties for the plotted sample, the $\odot$ symbol marks the Solar age and [Ce/Mg] value, and the $\otimes$ symbol represents the Solar $s$-process fraction \citep{arlandini_neutron_1999}.}
    \label{fig:model-comparison}
\end{figure}

Figure \ref{fig:model-comparison} compares the predicted [Ce/Mg]--age relation from our models against observed trends for Solar-metallicity stars in the Solar neighborhood. The fiducial model starts at the CCSN plateau of $\bracket[Mg]{Ce}_{\rm CC}\approx -0.64$ and rises steadily to a maximum of $\bracket[Mg]{Ce}\approx+0.2$ at a lookback time of 7 Gyr, then slowly declines by $\sim0.1\dex$ to the present day. The model completely fails to reproduce the trend observed in the data, over-predicting [Ce/Mg] for the oldest stars and under-predicting for the youngest. The model also fails to reproduce the Solar [Ce/Mg] at the age of the Sun, which could be fixed by scaling down the AGB yield by $\sim30\%$, although this would worsen the under-prediction for the youngest stars. \todo{This may not be as big of a problem if the Sun was born at a smaller radius, where [Ce/Mg] ratios are lower.}

We explore two modifications to the star formation history in Figure \ref{fig:model-comparison}. First, we reduce the SFE by half (i.e., doubling $\tau_\star$), which leads to a more gradual increase in [Ce/Mg] over time \todo{(why?)}. This alleviates the over-prediction problem for old stars, but results in a [Ce/Mg]--age relation that is flat, not increasing, over the past $5\Gyr$. Reducing the SFE also worsens agreement with the data in the [Fe/Mg]--[Mg/H] plane. \todo{Second, we add a Gaussian burst in the SFR.}
...
\begin{figure*}
    ...
    \caption{Stellar abundance distributions across the Galaxy predicted by the best model. The layout is similar to Figure \ref{fig:median-trends-grid}. Each point represents a model stellar population, color-coded by age. For visual clarity, only a random sample of 10,000 points is plotted in each panel. The dashed curve plots the gas abundance track at the mean radius in each bin. The solid contours show the 10th, 30th, 50th, 70th, and 90th percentiles of the MWM data in each region.}
\end{figure*}

% \begin{figure}
%     \centering
%     \includegraphics[width=\linewidth]{figures/onezone_sfh.pdf}
%     \caption{The effect of GCE model parameters on the predicted age--[Ce/Mg] relation. In each panel, the gray filled (open) points are low-Ia (high-Ia) stars in the Solar neighborhood ($7\leq R_g<9\kpc$, $z_{\rm max}<0.5\kpc$) with $-0.1\leq\bracket{Mg}<+0.1$. The $\odot$ symbol marks the Solar age and [Ce/Mg], and the gray error bars represent the median uncertainties for the plotted sample. The colored curves plot the gas abundance evolution of various GCE models, with the blue curve representing the same model in each panel. {\bf (a)} The star formation efficiency timescale $\tau_\star$. {\bf (b)} The outflow mass-loading factor $\eta$. {\bf (c)} The form of the star formation history; the inset plot in the bottom right shows the normalized star formation rate of each SFH.}
%     \label{fig:onezone-sfh}
% \end{figure}

% \begin{figure}
%     \centering
%     \includegraphics[width=\linewidth]{figures/onezone_agb.pdf}
%     \caption{The effect of adjustments to AGB yield predictions on GCE model predictions. The data points are the same as in Figure \ref{fig:onezone-sfh}. The colored curves plot the gas abundance evolution of various GCE models, with the blue curve representing the same model in each panel. {\bf (a)} Global enhancement of the AGB yields; the dashed curve is a model without a prompt Ce enrichment channel. The ``$\otimes$'' symbol represents the Solar $s$-process fraction \citep{arlandini_neutron_1999}. {\bf (b)} Multiplicative scaling of the AGB progenitor masses. {\bf (c)} Multiplicative scaling of the AGB progenitor metallicities.}
%     \label{fig:onezone-agb}
% \end{figure}

% \begin{figure}
%     \centering
%     \includegraphics[width=\linewidth]{figures/delayed_enrichment_models.pdf}
%     \caption{\todo{Probably cut or significantly revamp this figure.}}
