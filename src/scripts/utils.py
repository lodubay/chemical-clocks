...
def import_sample(good_ages=True, cut_limits=True, verbose=False):
    """
    Import the MWM sample file generated by sample.py.
    
    Parameters
    ----------
    good_ages : bool [default: True]
        If True, cut stars with poor age estimates based on the training
        density (Stone-Martinez et al. 2025).
    cut_limits : bool [default: True]
        If True, cut stars with abundance measurements below the APOGEE upper 
        limits (Shetrone et al. 2025).
    verbose : bool [default: False]
        If True, print verbose output to terminal.
    """
    sample = pd.read_csv(paths.data / 'sample.csv', index_col='sdss_id')
    if good_ages:
        sample = sample[sample['good_age']]
    if cut_limits:
